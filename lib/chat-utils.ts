...
export function parseMessageContent(content: string): any | null {
  ...
  if (!content || typeof content !== 'string') {
    console.warn('parseMessageContent: 無効な入力', content);
    return null;
  }
  ...
  try {
    ...
    console.log('JSON直接解析成功:', Object.keys(result).join(', '));
  } catch (error) {
    // 失敗した場合は別の方法を試す
    try {
      ...
      if (jsonMatch) {
        ...
        console.log('マークダウンJSON解析成功:', Object.keys(result).join(', '));
      } 
      // JSONが途中で切れている場合の処理
      else if (content.includes('{ "content": "') && !content.endsWith('"}')) {
        // 不完全なJSONを修正して完全なJSONにする
        console.log('不完全なJSONを修正します');
      ...
      // コンテンツからJSONっぽい部分を抽出
        ...
        if (jsonBlockMatch) {
          // マッチしたJSON部分を解析
          console.log('JSONブロックを抽出して解析します');
          const extractedJson = jsonBlockMatch[0].replace(/\n/g, '\\n');
          result = JSON.parse(extractedJson);
        }
      }
    } catch (nestedError) {
      // すべての方法が失敗した場合は詳細なログを出力
      console.error('JSON解析に失敗:', nestedError);
      console.error('解析失敗コンテンツ:', content.length > 200 ? content.substring(0, 200) + '...' : content);
      result = null;
    }
  }
  ...
  if (result && typeof result === 'object') {
    // content フィールドがない場合は追加
    if (!result.content) {
      console.warn('JSONにcontent属性がないため、デフォルト値を設定します');
      result.content = '応答を処理できませんでした。もう一度お試しください。';
    }
  }
...
export function extractChoicesFromText(content: string): { choices: string[], descriptions: string[] } {
  ...
  // 入力バリデーション
  if (!content || typeof content !== 'string') {
    console.warn('extractChoicesFromText: 無効な入力', content);
  ...
  for (const pattern of patterns) {
    ...
    if (matches.length > 0) {
      console.log(`選択肢パターンに一致: ${matches.length}件`);
...
export function shouldSplitMessage(content: string, choices: string[] = []): boolean {
  ...
  return (content.length > 20) || choices.length > 0;
}
...
export function processMessage(content: string): {
  ...
  console.log('処理中のメッセージ:', content ? content.substring(0, 50) + (content.length > 50 ? '...' : '') : '空');
  ...
  if (isJsonContent) {
    try {
      ...
      if (parsedJson.choices && Array.isArray(parsedJson.choices)) {
        formattedChoices = parsedJson.choices.map((choice: any, index: number) => 
      ...
      // 選択肢がJSONになく、テキスト中に埋め込まれている場合は抽出を試みる
      ...
      console.log('JSON解析に失敗したため、テキストとして処理します');
    }
  }
...
export const splitMessageIntoParts = (
  ...
  // 入力検証
  if (!content) {
    console.warn('splitMessageIntoParts: 空の入力');
    return [{
      content: '応答を処理できませんでした。もう一度お試しください。',
      shouldSplit: false
    }];
  }
...
export const extractRecipe = (content: string, currentPhase?: ChatPhase): any | null => {
  if (!content || !currentPhase || !['finalized', 'complete'].includes(currentPhase)) {
    return null;
  }
